...
const { admin } = require("../config/firebaseConfig");
const { createUserInFirestore, getUserByUid } = require("../models/userModel");
const { putBlob } = require("../storage/vercelBlob");
const axios = require("axios");
const path = require("path");
...
const registerUser = async (req, res) => {
  try {
    ...
    const {
      firstName, lastName, email, username, employeeId, nic, password, type, adminRole,
    } = req.body;
    ...
    // 1) Create Firebase Auth user first (to use uid in the blob key)
    const userRecord = await admin.auth().createUser({
      email,
      password,
      displayName: `${firstName} ${lastName}`,
    ...
    // 2) If a file came in, push it to Vercel Blob
    let profileImage = "";
    if (req.file?.buffer) {
      const ext = path.extname(req.file.originalname || "").toLowerCase() || ".bin";
      const key = `profiles/${userRecord.uid}_${Date.now()}${ext}`;
      profileImage = await putBlob(key, req.file.buffer, req.file.mimetype || "application/octet-stream");
    }
    ...
    const userData = {
      ...
      nic,
      type,
      adminRole: type === "admin" ? (adminRole || "editor") : undefined,
      profileImage,               // <-- Blob public URL
      createdBy: currentUserUid,
      createdAt: new Date().toISOString(),
      isActive: true,
    };
    ...
    res.status(201).json({ message: "User registered successfully", userId: userRecord.uid, profileImage });
  } catch (error) {
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    const userData = await getUserByUid(localId);
    if (!userData) return res.status(404).json({ error: "User not found in Firestore" });
    res.status(200).json({ message: "Login successful", user: { ...userData, uid: localId }, token: idToken });
  } catch (error) {
    const msg = error.response?.data?.error?.message || error.message;
    res.status(401).json({ error: msg });
  }
